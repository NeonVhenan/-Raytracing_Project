...
bool shadowFeeler(vec4 p0, Object* object) {
	...
	for (unsigned int i = 0; i < sceneObjects.size(); i++) {
		...
		float lightIntersect = sceneObjects[i] -> intersect(lightPosition, p0 - lightPosition).t;

		if ((lightIntersect + EPSILON) < 1.0) { //EPSILON -> round off double to avoid precision errors
	...
	//Soft shadows
	int nbSample = 10; //to be more precise -> increase nbSample (test 100)
	nbTrue = 1;
	std::vector < point4 > areaLight; //nbSample * nbSample

	for (unsigned int i = 0; i < nbSample; i++) {

		for (unsigned int j = 0; j < nbSample; j++) {

			areaLight.push_back(point4(lightPosition.x + (float)i / (float)nbSample,
				lightPosition.y,
				lightPosition.z + (float)j / (float)nbSample,
				lightPosition.w));
		}
	...
	for (unsigned int i = 0; i < sceneObjects.size(); i++) {
		...
		for (unsigned int j = 0; j < (nbSample * nbSample); j++) {

			float lightIntersect = sceneObjects[i] -> intersect(areaLight[j], p0 - areaLight[j]).t;

			if ((lightIntersect + EPSILON) < 1.0) { //EPSILON -> round off double to avoid precision errors

				inShadow = true;
				nbTrue -= 1.0 / (nbSample * nbSample);
			}
...
vec4 castRay(vec4 p0, vec4 E, Object* lastHitObject, int depth) {
	...
	for (int i = 0; i < sceneObjects.size(); i++) {
		...
		if (intersections[i].t < distMin) {

			distMin = intersections[i].t;
			id_object = intersections[i].ID_;
		}
	}
	...
	//Phong + Specular
	vec4 pos = intersections[id_object].P; //intersections[result.ID_].P;

	vec4 N = normalize(intersections[id_object].N); //normalize(intersections[id_object].N) ; norm
	vec4 V = normalize(E); //Ray direction
	vec4 L = normalize(lightPosition - pos); //Towards the light? 
	vec4 R = normalize(reflect(L, N)); //Towards the camera?

	vec4 R_mirror = normalize(reflect(E, N)); //mirror ; nop xD  normalize(reflect(-(cameraPosition - intersections[id_object].P), N));
	// vec4 R_glass = ????; //glass

	vec4 rayReflection = reflect(L, N);

	color4 material_ambient(lightColor.x * sceneObjects[id_object] -> shadingValues.Ka,
		lightColor.y * sceneObjects[id_object] -> shadingValues.Ka,
		lightColor.z * sceneObjects[id_object] -> shadingValues.Ka, 1.0);

	color4 material_diffuse(lightColor.x,
		lightColor.y,
		lightColor.z, 1.0);

	color4 material_specular(sceneObjects[id_object] -> shadingValues.Ks,
		sceneObjects[id_object] -> shadingValues.Ks,
		sceneObjects[id_object] -> shadingValues.Ks, 3.0);

	float  material_shininess = sceneObjects[id_object] -> shadingValues.Kn;

	color4 ambient_product = GLState::light_ambient * material_ambient;
	color4 diffuse_product = (GLState::light_diffuse * material_diffuse) * fmax(dot(L, N), 0.0f);
	color4 specular_product = (GLState::light_specular * material_specular) * pow(fmax(dot(V, R), 0.0f), material_shininess);

	color = sceneObjects[id_object] -> shadingValues.color * (ambient_product + diffuse_product +specular_product);

	/*//Hard shadows
	if (shadowFeeler(pos, lastHitObject) == true) {
	...
	//Mirror
	if (sceneObjects[id_object] -> shadingValues.Ks > 0) {

		color = castRay(pos - EPSILON, R_mirror - EPSILON, lastHitObject, depth + 1);
	}

	//Glass
	if (sceneObjects[id_object]->shadingValues.Kt != 0) {

		double cos_theta = dot(V, N);
		double sin_theta = sqrt(1.0 - (cos_theta * cos_theta));

		//reflection
		vec4 reflected = cos_theta * N; // -> soap bubble ; extreme transparency (no color at all) (oblivion.png)
		//vec4 reflected = castRay(pos - EPSILON, R_mirror - EPSILON, lastHitObject, depth + 1); // -> not transparent but a weird reflection (unknown.png)
		//vec4 reflected = cos_theta * N * castRay(pos - EPSILON, R_mirror - EPSILON, lastHitObject, depth + 1); // -> nothing interesting
		//vec4 reflected = castRay((cos_theta * N) - EPSILON, R - EPSILON, lastHitObject, depth + 1); // -> nothing interesting

		//refraction
		vec4 refracted = sqrt(1.0 - (cos_theta * cos_theta)); // -> soap bubble (soap_bubble.png) ; extreme transparency (no color at all) (oblivion.png)
		//vec4 refracted = (sin_theta * sin_theta) * (1.0 - (cos_theta * cos_theta)); //-> a "transparent-ish" (aka no color at all) sphere with a white halo around (shining_2.png)
		//vec4 refracted = castRay((sin_theta * sin_theta) * (1.0 - (cos_theta * cos_theta)) - EPSILON, R, lastHitObject, depth + 1); // -> not transparent but a weird reflection (unknown.png)
		//vec4 refracted = castRay(sin_theta - EPSILON, R - EPSILON, lastHitObject, depth + 1); // -> nothing interesting

		color = sceneObjects[id_object] -> shadingValues.Kr * reflected + (1 - sceneObjects[id_object] -> shadingValues.Kr) * refracted; // -> soap bubble (soap_bubble.png)
		//color = sceneObjects[id_object] -> shadingValues.Kt * reflected + (1 - sceneObjects[id_object] -> shadingValues.Kt) * refracted;  // -> extreme transparency (no color at all) (oblivion.png)
		//color = castRay((sceneObjects[id_object] -> shadingValues.Kt * reflected + (1 - sceneObjects[id_object] -> shadingValues.Kt) * refracted), R - EPSILON, lastHitObject, depth + 1); // -> nothing interesting
		//color = (sceneObjects[id_object] -> shadingValues.Ks * reflected + (1.0 - sceneObjects[id_object] -> shadingValues.Ks)) * ((sceneObjects[id_object] -> shadingValues.Kt) * refracted + (1.0 - sceneObjects[id_object] -> shadingValues.Kt) * color); // -> a "transparent-ish" (aka no color at all) sphere with a white halo around (shining_2.png)
		//color = castRay((sceneObjects[id_object] -> shadingValues.Ks * reflected + (1.0 - sceneObjects[id_object] -> shadingValues.Ks)) * ((sceneObjects[id_object] -> shadingValues.Kt) * refracted + (1.0 - sceneObjects[id_object] -> shadingValues.Kt) * color), R, lastHitObject, depth + 1); // -> not transparent but a weird reflection (unknown.png)
	}
...
void initCornellBox() {
	...
	{ //Ceiling
		...
		Object::ShadingValues _shadingValues;
		_shadingValues.color = vec4(0.5, 0.5, 1.0, 0.0);
		_shadingValues.Ka = 0.0;
		_shadingValues.Kd = 1.0;
		_shadingValues.Ks = 0.0;
		_shadingValues.Kn = 16.0;
	...
	{ //Front Wall
		...
		Object::ShadingValues _shadingValues;
		_shadingValues.color = vec4(1.0, 1.0, 1.0, 0.0);
		_shadingValues.Ka = 0.0;
		_shadingValues.Kd = 1.0;
		_shadingValues.Ks = 0.0;
		_shadingValues.Kn = 16.0;
	...
	{
		sceneObjects.push_back(new Sphere("Mirrored Sphere", vec3(-0.97, -1.25, -0.9), 0.75));
		Object::ShadingValues _shadingValues;
...
void initUnitSphere() {
	...
	lightColor = color4(1.0, 1.0, 1.0, 1.0); // red  color4(1.0, 0.0, 0.0, 1.0) ; white color4(1.0, 1.0, 1.0, 1.0)
...

	{
		...
		Object::ShadingValues _shadingValues;
		_shadingValues.color = vec4(1.0, 0.0, 0.0, 1.0); // white vec4(1.0, 1.0, 1.0, 1.0); red : vec4(1.0, 0.0, 0.0, 1.0);
		_shadingValues.Ka = 0.0;
		_shadingValues.Kd = 1.0;
		_shadingValues.Ks = 0.0;
		_shadingValues.Kn = 16.0;
...
int main(void) {
	...
	case _SPHERE:
		...
		{
			sceneObjects.push_back(new Sphere("Diffuse sphere", { -1., 0., -1. }, 1.5));
			Object::ShadingValues _shadingValues;
			_shadingValues.color = vec4(0.0, 1.0, 0.0, 0.0); // white vec4(1.0, 1.0, 1.0, 1.0); red : vec4(1.0, 0.0, 0.0, 1.0);
			_shadingValues.Ka = 0.0;
			_shadingValues.Kd = 1.0;
			_shadingValues.Ks = 0.0;
			_shadingValues.Kn = 16.0;
